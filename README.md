...
1. This tutorial uses the Push variation of the Observer Pattern. The publisher prepares a `Notification` payload and actively sends it to each subscriber through an HTTP POST request to the subscriber URL. The subscriber does not ask for the data on its own. Instead, the publisher pushes the event data directly when a product is created, deleted, or promoted.

2. If we used the Pull variation instead, one advantage would be that the publisher could stay simpler because it would only need to signal that something changed, while subscribers could decide when and what data to fetch. This could also reduce the amount of duplicated payload sent by the publisher and give subscribers more control over retries and data retrieval. However, for this tutorial case, Pull would also introduce important drawbacks. It would require extra HTTP requests, increase latency, and make subscribers more dependent on publisher endpoints. It is also less suitable for delete events, because after a product is deleted, a subscriber that tries to pull the latest data may no longer be able to retrieve the product details. In this case, Push is more practical because the publisher can send the relevant snapshot immediately when the event happens.

3. If we do not use multi-threading in the notification process, the program will still work functionally, but notification delivery will become synchronous and slower. The request that triggers the event, such as create, delete, or publish, would have to wait until notifications to all subscribers are sent one by one. If one subscriber is slow or unreachable, the whole process becomes delayed, and the client that called the publisher endpoint may experience a long response time. With many subscribers, this also reduces throughput because the publisher handles notification work serially instead of letting each delivery run independently.
